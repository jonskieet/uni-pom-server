...
import { BrandService, CategoryService, ProductService, SolutionService, PomService, FormTemplateService } from '../services'
...
// ── useFormTemplates — tải tất cả form templates ─────────────
export function useFormTemplates() {
  const { state, setData, setLoading, setError } = useAsync<any[]>([])

  const load = useCallback(async () => {
    setLoading()
    try {
      const r = await FormTemplateService.getAll()
      setData(Array.isArray(r) ? r : (r as any)?.data ?? [])
    }
    catch { setError('Không thể tải danh sách form template.') }
  }, [])
...
// ── useFormTemplate — tải form templates theo solutionId ─────
export function useFormTemplate(solutionId: number) {
  const { state, setData, setLoading, setError } = useAsync<any[]>([])

  const load = useCallback(async () => {
    setLoading()
    try {
      const r = await FormTemplateService.getAll(solutionId)
      setData(Array.isArray(r) ? r : (r as any)?.data ?? [])
    }
    catch { setError('Không thể tải form template.') }
  }, [solutionId])
