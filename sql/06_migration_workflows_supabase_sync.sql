-- UNI EMS - Synchronize the Workflow schema on Supabase.
-- Safe to run more than once. Run after migration_workflows.sql.
...
BEGIN
  IF to_regclass('public.workflows') IS NULL
     ...
     OR to_regclass('public.workflow_instance_steps') IS NULL THEN
    RAISE EXCEPTION
      'Base Workflow tables are missing. Run sql/migration_workflows.sql first.';
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_workflow_instances_due_date
  ON workflow_instances(due_date)
...
DO $$
BEGIN
  ALTER TABLE pom_construction_logs
    ADD CONSTRAINT pom_construction_logs_type_check
...
ALTER TABLE pom_construction_logs
  VALIDATE CONSTRAINT pom_construction_logs_type_check;

CREATE INDEX IF NOT EXISTS idx_pom_construction_logs_pom
  ON pom_construction_logs(pom_id, created_at DESC);

-- Repair old instances whose first step was never activated.
WITH stuck AS (
  SELECT wi.id
  ...
  WHERE wi.status = 'in_progress'
    AND EXISTS (
      ...
      WHERE x.instance_id = wi.id
    )
    AND NOT EXISTS (
      SELECT 1 FROM workflow_instance_steps x
      WHERE x.instance_id = wi.id
        AND x.status IN ('in_progress', 'completed')
    )
), first_steps AS (
  SELECT DISTINCT ON (wis.instance_id)
    wis.instance_id,
    wis.step_id
...
SET status = 'in_progress',
    started_at = COALESCE(wis.started_at, NOW()),
    updated_at = NOW()
FROM first_steps fs
WHERE wis.instance_id = fs.instance_id
  AND wis.step_id = fs.step_id;

-- A won BOM enters construction; only project_completed closes the project.
...
SELECT
  ...
  CASE
    WHEN p.status::text = 'project_completed' THEN 'completed'
    WHEN p.status::text = 'closed_lost' THEN 'cancelled'
    ELSE 'in_progress'
  END AS status,
